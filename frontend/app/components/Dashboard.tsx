...
export default function Dashboard() {
  ...
  return (
    <div className={cn(
      ...
      {/* Quiet Header - Hide when full screen */}
      {!isFullScreen && (
            ...
            <div className="p-8 rounded-[2rem] bg-white/[0.01] border border-white/[0.03] flex flex-col items-center gap-6 backdrop-blur-sm">
              <div className="flex flex-col items-center">
                <div className="h-16 w-16 rounded-full bg-primary/10 flex items-center justify-center text-3xl">
                  {lastMotivation <= 20 ? "😴" : lastMotivation <= 40 ? "☕" : lastMotivation <= 60 ? "🧘" : lastMotivation <= 80 ? "🚀" : "⚡"}
                </div>
              </div>
              ...
              <div className="text-center font-black h-8 flex items-center mt-4">
                <span className="text-lg text-primary uppercase tracking-tighter leading-tight">
                  {lastMotivation <= 20 ? "Deep Chill" : lastMotivation <= 40 ? "Chill" : lastMotivation <= 60 ? "Balanced" : lastMotivation <= 80 ? "Lock-in" : "Deep Lock-in"}
                </span>
              </div>
            </div>
                ...
                <ScrollArea className="flex-1 p-10 space-y-6 custom-scrollbar">
                  {chatMessages.map(m => (
                    <div key={m.id} className={cn("flex", m.role === "user" ? "justify-end" : "justify-start")}>
                      <div className={cn("max-w-[85%] p-5 rounded-[1.5rem] text-base font-medium leading-relaxed", m.role === "user" ? "bg-primary text-primary-foreground rounded-tr-none" : "bg-white/[0.03] border border-white/5 rounded-tl-none")}>
                        {m.content}
                      </div>
                    </div>
                  ))}
