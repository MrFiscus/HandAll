...
const HOUR_HEIGHT = 42; // Compact view
...
export default function WeeklyCalendar() {
  ...
  const navigate = (dir: "prev" | "next" | "today") => {
    if (dir === "today") setCurrentWeekStart(startOfWeek(new Date(), { weekStartsOn: 0 }));
    else setCurrentWeekStart(prev => addDays(prev, dir === "next" ? 7 : -7));
  };
  ...
  const getPositionedEvents = (day: Date): PositionedEvent[] => {
    ...
    const dayEvents = events
      ...
      .map(e => {
        ...
        return {
          ...e,
          // Virtual times for positioning calculation only
  ...
  const saveEvent = async () => {
    ...
    try {
      if (dialogMode === "create") {
        await addEvent({ title: form.title, start, end, type: form.type, xpValue: form.type === "working" ? 50 : 10 });
    ...
    <div className="flex flex-col h-full bg-card border rounded-xl overflow-hidden shadow-md">
      ...
      <div className="flex items-center justify-between p-4 border-b bg-muted/5">
        ...
        <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground bg-secondary/50 px-3 py-1.5 rounded-full uppercase tracking-tighter">
          <Clock className="h-3 w-3" /> Week View
        </div>
      </div>
      ...
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto relative bg-background select-none">
        <div className="grid grid-cols-[60px_1fr] min-h-0">
          ...
          <div className="grid grid-cols-7 relative h-[1008px]">
            {weekDays.map(day => {
                ...
                <div key={day.toISOString()} className={cn("relative border-r last:border-r-0 h-full", isSameDay(day, new Date()) && "bg-primary/[0.02]")}
                  ...
                  {dayEvents.map(event => (
                    <div key={event.id} draggable onDragStart={(e) => onDragStart(e, event.id)} onDragEnd={() => {setDraggingId(null); setDragPreview(null);}}
                         className={cn("event-block absolute rounded-md border-l-4 p-1.5 text-[10px] leading-tight overflow-hidden shadow-sm z-10 transition-all cursor-pointer hover:z-20", getColor(event.type), draggingId === event.id && "opacity-30")}
                      ...
                      <div className="opacity-80 font-bold">{format(new Date(event.start), "h:mm a")}</div>
                    </div>
                  ))}
            ...
            <TimeIndicator currentWeekStart={currentWeekStart} />
          </div>
        </div>
      </div>
...
function TimeIndicator({ currentWeekStart }: { currentWeekStart: Date }) {
